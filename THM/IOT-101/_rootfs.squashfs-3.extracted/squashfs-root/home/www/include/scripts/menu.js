...
var menuClass = Class.create({
	data: $H({
		'Configuration': $H({
			"System": $H({
				"Basic": ["General", "Time"],
				"Advanced": ["General", "Hotspot", "Syslog", "Ethernet"]
			}),
			"IP": $H({
				"IP Settings": [],
			...
			"Wireless": $H({
				"Basic": ["Wireless Settings", "Scheduled Wireless ON-OFF", "QoS Settings"],
				"Advanced": ["Wireless Settings", "QoS Settings"]
			}),
			"Security": $H({
				"Profile Settings": [],
	...
	processConfig: function() {
		...
		if(!config.SCH_WIRELESS_ON_OFF.status){
			this.data.get("Configuration").get("Wireless").get("Basic").removeItem("Scheduled Wireless ON-OFF");
		}
