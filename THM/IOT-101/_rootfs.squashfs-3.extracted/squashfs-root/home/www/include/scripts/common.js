...
//alert("After tab Switch:"+"\n"+'activeMode = '+this.activeMode+"\nradioId="+this.radioId+"\nselectedRadioId="+$S(this.radioId));
    ...
    getModeString: function() {
        this.modeString = new Array('b','bg','ng','a','na');
        if (!config.TWOGHZ.status) {
            this.modeString.splice(0, 3, "");
        }
        if (!config.FIVEGHZ.status) {
            this.modeString.splice(3,2, "");
        }
    },
    ...
    setActiveMode: function(event,flag)
    {
        ...
        if (currentRadio == -1) {
            ...
            this.getModeString();
            var flag1 = true;
//alert("activeMode = "+this.activeMode+"\ncurrentRadio="+currentRadio +"\nflag=="+flag);
            if (String(this.activeMode) != String(currentRadio) && flag && (this.checkEmptyActiveMode() == false)) {
                if(!config.DUAL_CONCURRENT.status){
                    if(this.modeString[this.activeMode] == undefined){
                        //var flag1 = confirm("Do you want to disable 11"+this.modeString[$('activeMode'+String((this.id == 1)?(this.id):(this.id-2))).value]+" mode and enable 11"+this.modeString[currentRadio]+" mode?\n\nNote: Click APPLY for the changes to take effect.");
                        var flag1 = true;
                    }
                    else {
    ...
    setLabelText: function(currentRadio) {
        ...
        countMax = (config.TWOGHZ.status)?(config.MODE11N.status?2:1):0;
        if (config.DUAL_CONCURRENT.status) {
...
function redirectHome(sessionCheck,username)
{
	var flag = false;
	if (sessionCheck) {
		...
		if (flag) {
			var oOptions = {
							method: "get",
				asynchronous: false,
				parameters: { username: username, id: Math.floor(Math.random()*1000001) },
...
function doLogin(event)
{
	...
	var oOptions = {
			...
			parameters: { username: $('username').value, password: $('password').value, id: Math.floor(Math.random()*1000001) },
				...
				if(response == 'loginok') {
					//alert("Login OK!");
					redirectHome(false,username);
				}
				else if (response == 'sessionexists') {
					redirectHome(true,username);
				}
				else {
					if (parent.parent.frames['master'].progressBar != undefined)
...
function doPacketCapture(actionToDo)
{
	...
	var req = new Ajax.Request('packetCapture.php?id='+Math.random(10000,99999), oOptions);
...
function doSubmit(obj, val, changed)
{
	...
	if ( $('firmwareFile') != undefined) {
		...
		if (checkSubmit(val)){
			...
			else if (($('priRadIpAddr') != undefined) && ($('authType') != undefined) && $('priRadIpAddr').value == '' && (($('authType').value == '0')) ) {
